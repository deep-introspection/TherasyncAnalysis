...
class HRCleaner:
    ...
    def __init__(self, config_path: Optional[Union[str, Path]] = None):
        ...
        self.interpolation_max_gap = hr_config.get(
    ...
    def _interpolate_gap(
        ...
        if pd.isna(data.loc[before_idx, "hr_clean"]) or pd.isna(
            data.loc[after_idx, "hr_clean"]
        ):
            return
        ...
        interpolated_hr = np.interp(
            time_points, [start_time, end_time], [start_hr, end_hr]
        )
    ...
    def _calculate_quality(self, data: pd.DataFrame) -> pd.Series:
        ...
        low_edge = (data["hr_clean"] < min_hr + edge_threshold) & (
            data["hr_clean"] >= min_hr
        )
        high_edge = (data["hr_clean"] > max_hr - edge_threshold) & (
            data["hr_clean"] <= max_hr
        )
