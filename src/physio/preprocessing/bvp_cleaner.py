...
class BVPCleaner:
    ...
    def __init__(self, config_path: Optional[Union[str, Path]] = None):
        ...
        self.method_quality = self.processing_config.get(
        ...
        logger.info(
            f"BVP Cleaner initialized with method={self.method}, quality={self.method_quality}"
        )
    ...
    def process_moment_signals(
        self, moment_data: Dict[str, Union[pd.DataFrame, Dict]]
        ...
        for moment, moment_info in moment_data.items():
            try:
                ...
                sampling_rate = metadata.get(
                    "SamplingFrequency", self.bvp_config.get("sampling_rate", 64)
                )
    ...
    def _validate_input_signal(
        self, signal: np.ndarray, sampling_rate: int, moment: str
    ) -> None:
        """
    ...
    def _validate_processing_results(
        ...
        missing_columns = [
            col for col in required_columns if col not in processed_signals.columns
        ]
        if missing_columns:
            raise ValueError(
    ...
    def apply_additional_filtering(
        self, signal: Union[pd.Series, np.ndarray], sampling_rate: int
    ) -> np.ndarray:
        """
        ...
        try:
            ...
            logger.warning(
                f"Additional filtering failed: {e}, returning original signal"
            )
