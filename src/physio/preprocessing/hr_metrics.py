...
class HRMetricsExtractor:
    ...
    def extract_metrics(
        ...
        """
        logger.info(
            f"Extracting HR metrics for moment '{moment}' ({len(data)} samples)"
        )
        ...
        quality_values = (
            valid_data[quality_col].values
            if quality_col in valid_data.columns
            else np.ones(len(valid_data))
        )
        ...
        metrics = {
            ...
            "contextual": self._extract_contextual_metrics(
                data, valid_data, quality_values
            ),
        }
    ...
    def _extract_descriptive_metrics(self, hr_values: np.ndarray) -> Dict[str, float]:
        ...
        return {
            ...
            "hr_iqr": float(
                np.percentile(hr_values, 75) - np.percentile(hr_values, 25)
            ),
        }
    ...
    def _extract_response_metrics(
        ...
        baseline_duration = min(
            self.baseline_window, (time_values[-1] - time_values[0]) / 3
        )
        baseline_mask = time_values <= (time_values[0] + baseline_duration)
        baseline_hr = (
            np.mean(hr_values[baseline_mask])
            if baseline_mask.any()
            else np.mean(hr_values)
        )
        ...
        response_latency = (
            time_values[response_mask][0] - time_values[0]
    ...
    def extract_session_metrics(
        ...
        """
        logger.info(
            f"Extracting HR metrics for {len(processed_results)} moments (dict format)"
        )
