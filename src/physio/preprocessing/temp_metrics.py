...
class TEMPMetricsExtractor:
    ...
    def extract_metrics(
        ...
        """
        logger.info(
            f"Extracting temperature metrics for moment '{moment}' ({len(data)} samples)"
        )
        ...
        metrics = {
            ...
            "contextual": self._extract_contextual_metrics(
                data, valid_data, quality_values, clean_col
            ),
        }
    ...
    def _extract_descriptive_metrics(self, temp_values: np.ndarray) -> Dict[str, float]:
        ...
        return {
            ...
            "temp_std": float(np.std(temp_values, ddof=1))
            ...
            "temp_iqr": float(
                np.percentile(temp_values, 75) - np.percentile(temp_values, 25)
            ),
        }

    def _extract_trend_metrics(
        self, temp_values: np.ndarray, time_values: np.ndarray
    ) -> Dict[str, float]:
        """
        ...
        return {
            ...
            "temp_slope_per_minute": float(
                slope * 60
            ),  # °C per minute (more interpretable)
    ...
    def _extract_stability_metrics(self, temp_values: np.ndarray) -> Dict[str, float]:
        ...
        return {"temp_stability": float(stability), "temp_cv": float(cv)}
    ...
    def _empty_metrics(self, moment: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "stability": {"temp_stability": nan_value, "temp_cv": nan_value},
            "contextual": {
                "temp_duration": 0.0,
    ...
    def extract_session_metrics(
        self, processed_results: Dict[str, pd.DataFrame]
        ...
        """
        logger.info(
            f"Extracting temperature metrics for {len(processed_results)} moments (dict format)"
        )
