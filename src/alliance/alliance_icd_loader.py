...
class AllianceICDLoader:
    ...
    def load_alliance_states(
        self, group_id: str, session_id: str, method: str = "nsplit"
    ) -> Dict[int, int]:
        """
    ...
    def _compute_alliance_states(
        self, df: pd.DataFrame, epoch_col: str
    ) -> Dict[int, int]:
        """
        ...
        for epoch_id in epochs_with_annotations:
            overlapping = df[
                df[epoch_col].apply(
                    lambda x: epoch_id in x if isinstance(x, list) else False
                )
            ]
    ...
    def load_icd_data(
        self, dyad_type: str = "intra", task: str = "therapy", method: str = "nsplit120"
    ) -> pd.DataFrame:
        """
        ...
        if dyad_type == "intra":
            ...
            icd_file = (
                dppa_dir
                / "intra_family"
                / f"intra_family_icd_task-{task}_method-{method}.csv"
            )
        else:
            icd_file = (
                dppa_dir
                / "inter_session"
                / f"inter_session_icd_task-{task}_method-{method}.csv"
            )
    ...
    def get_valid_dyads(
        self, sessions_with_moi: Optional[Dict[str, List[str]]] = None
    ...
    def merge_alliance_icd(
        self, dyad_column: str, icd_df: pd.DataFrame, dyad_type: str = "real"
    ) -> pd.DataFrame:
        """
        ...
        try:
            alliance_states = self.load_alliance_states(
                family, session, method="nsplit"
            )
        except FileNotFoundError:
