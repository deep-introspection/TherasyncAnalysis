...
def plot_eda_arousal_profile(
    ...
    bp = ax1.boxplot(
        ...
        showmeans=True,
        meanprops=dict(
            marker="D", markerfacecolor="white", markeredgecolor="black", markersize=8
        ),
    ...
    ax1.set_ylabel(
        "EDA Tonic Level (µS)", fontsize=FONTSIZE["label"], fontweight="bold"
    )
    ax1.grid(True, alpha=ALPHA["fill"], axis="y", linestyle="--")
    ax1.set_title(
        "Baseline Arousal Distribution\n(Tonic EDA)",
    ...
    for i, moment in enumerate(stats.keys()):
        ...
        ax2.text(
            i,
            s["phasic_std"] + max([stats[m]["phasic_std"] for m in stats]) * 0.02,
            f"{s['phasic_std']:.3f}",
            ha="center",
            va="bottom",
    ...
    ax2.set_ylabel(
        "EDA Phasic Std Dev (µS)", fontsize=FONTSIZE["label"], fontweight="bold"
    )
    ax2.set_xticks(x)
    ax2.set_xticklabels(
        [get_moment_label(m) for m in stats.keys()],
...
def plot_scr_distribution(
    ...
    if not amplitude_data:
        ...
        ax.text(
            0.5,
            0.5,
            "No SCR amplitude data available",
            ha="center",
            va="center",
            fontsize=FONTSIZE["label"],
        )
    ...
    fig, axes = plt.subplots(
        n_moments, 2, figsize=(FIGSIZE["wide"][0], FIGSIZE["wide"][1] * n_moments / 2)
    )
    ...
    for idx, (moment, amplitudes) in enumerate(amplitude_data.items()):
        ...
        ax_hist.set_xlabel(
            "Amplitude (µS)", fontsize=FONTSIZE["label"], fontweight="bold"
        )
        ax_hist.set_title(
            f"{get_moment_label(moment)} - Distribution (n={len(amplitudes)})",
            fontsize=FONTSIZE["subtitle"],
            fontweight="bold",
        )
        ...
        ax_box.set_title(
            f"{get_moment_label(moment)} - Statistics",
            fontsize=FONTSIZE["subtitle"],
            fontweight="bold",
        )
