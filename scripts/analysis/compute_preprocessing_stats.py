...
def find_all_preprocessed_sessions(
...
def extract_bvp_metrics(
    ...
    metrics = {"subject": subject, "session": session, "modality": "BVP"}
    ...
    try:
        ...
        if lfhf_max_key in metrics and pd.notna(metrics[lfhf_max_key]):
            if metrics[lfhf_max_key] > 10:
                ...
                logger.warning(
                    f"{subject}/{session}: High LF/HF ratio = {metrics[lfhf_max_key]:.2f}"
                )
        ...
        if hr_mean_max_key in metrics and pd.notna(metrics[hr_mean_max_key]):
            if metrics[hr_mean_max_key] > 200:
                ...
                logger.warning(
                    f"{subject}/{session}: Very high HR = {metrics[hr_mean_max_key]:.1f} bpm"
                )
        if hr_mean_min_key in metrics and pd.notna(metrics[hr_mean_min_key]):
            if metrics[hr_mean_min_key] < 30:
                ...
                logger.warning(
                    f"{subject}/{session}: Very low HR = {metrics[hr_mean_min_key]:.1f} bpm"
                )
...
def extract_eda_metrics(
    ...
    metrics = {"subject": subject, "session": session, "modality": "EDA"}
    ...
    try:
        ...
        for tonic_key in tonic_keys:
            if pd.notna(metrics[tonic_key]) and metrics[tonic_key] < 0:
                ...
                logger.warning(
                    f"{subject}/{session}: Negative EDA tonic ({tonic_key}) = {metrics[tonic_key]:.6f} µS"
                )
        ...
        for phasic_key in phasic_keys:
            if pd.notna(metrics[phasic_key]) and metrics[phasic_key] < 0:
                ...
                logger.warning(
                    f"{subject}/{session}: Negative EDA phasic ({phasic_key}) = {metrics[phasic_key]:.6f} µS"
                )
...
def extract_hr_metrics(
    ...
    metrics = {"subject": subject, "session": session, "modality": "HR"}
...
def compute_all_stats(config: Dict, logger: logging.Logger) -> pd.DataFrame:
    ...
    for subject, session in sessions:
        ...
        combined.update(
            {
                k: v
                for k, v in bvp_metrics.items()
                if k not in ["subject", "session", "modality"]
            }
        )
        combined.update(
            {
...
def identify_outliers(df: pd.DataFrame, logger: logging.Logger) -> pd.DataFrame:
    ...
    for idx, row in df.iterrows():
        ...
        if lfhf_col in df.columns and pd.notna(row[lfhf_col]):
            if row[lfhf_col] > 10:
                outlier_info["outlier_types"].append(
                    f"High LF/HF ({row[lfhf_col]:.2f})"
                )

        # Check for negative EDA Tonic (check all tonic min columns)
        tonic_min_cols = [
            c for c in df.columns if "EDA_EDA_Tonic" in c and "_min" in c.lower()
        ]
        for col in tonic_min_cols:
            if pd.notna(row[col]) and row[col] < 0:
                outlier_info["outlier_types"].append(
                    f"Negative {col.replace('EDA_EDA_', 'EDA_')} ({row[col]:.6f} µS)"
                )
                break  # Only report once per session

        # Check for negative EDA Phasic
        phasic_min_cols = [
            c for c in df.columns if "EDA_EDA_Phasic" in c and "_min" in c.lower()
        ]
    ...
    if len(outlier_df) > 0:
        ...
        for _, row in outlier_df.iterrows():
            logger.warning(
                f"{row['subject']}/{row['session']}: {row['outlier_summary']}"
            )
        logger.warning(f"{'=' * 80}\n")
    else:
...
def create_distribution_plots(
    df: pd.DataFrame, output_dir: Path, logger: logging.Logger
):
    ...
    available_metrics = [
        (col, label, xlim) for col, label, xlim in metrics_to_plot if col in df.columns
    ]
    ...
    for idx, (metric_col, label, xlim) in enumerate(available_metrics):
        ...
        # Histogram + KDE
        ax.hist(
            data, bins=30, alpha=0.6, color="steelblue", edgecolor="black", density=True
        )
        ...
        ax.set_title(
            f"{label} Distribution (n={len(data)})", fontsize=12, fontweight="bold"
        )
...
Examples:
    ...
    logger.info(
        f"Metrics extracted: {len(stats_df.columns) - 2}"
    )  # -2 for subject, session
