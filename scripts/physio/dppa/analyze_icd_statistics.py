...
def analyze_task(
    plotter: ICDStatsPlotter, icd_file: Path, task: str, output_dir: Path
) -> dict:
    """
    ...
    if metadata["n_pseudo_dyads"] == 0:
        logger.warning(
            f"No pseudo dyads found for {task}. Skipping corrected analysis."
        )
        return None
...
def main():
    ...
    parser.add_argument(
        "--task", choices=["therapy", "restingstate"], help="Task to analyze"
    )
    parser.add_argument("--all", action="store_true", help="Analyze all tasks")
    ...
    for task, results in all_results.items():
        ...
        if naive_p < 0.05 and corrected_p >= 0.05:
            print("  ⚠️  Significance LOST after correction!")
