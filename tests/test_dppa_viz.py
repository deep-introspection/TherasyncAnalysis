...
class TestDyadICDLoader:
    ...
    def test_load_icd_therapy_valid(self, loader, valid_dyad_pair):
        ...
        df = loader.load_icd(
            dyad_pair=valid_dyad_pair, task="therapy", method="nsplit120"
        )
    ...
    def test_load_both_tasks_valid(self, loader, valid_dyad_pair):
        ...
        result = loader.load_both_tasks(dyad_pair=valid_dyad_pair, methods=methods)
...
class TestDyadCentroidLoader:
    ...
    def test_load_centroids_therapy_valid(self, loader, valid_dyad_info):
        ...
        df1, df2 = loader.load_centroids(
            dyad_info=valid_dyad_info, task="therapy", method="nsplit120"
        )
    ...
    def test_load_centroids_missing_file(self, loader):
        ...
        with pytest.raises(FileNotFoundError):
            loader.load_centroids(
                dyad_info=invalid_dyad_info, task="therapy", method="nsplit120"
            )
    ...
    def test_validate_epoch_alignment_match(self, loader, valid_dyad_info):
        ...
        is_aligned_artificial = loader.validate_epoch_alignment(
    ...
    def test_validate_epoch_alignment_mismatch(self, loader):
        ...
        df2 = pd.DataFrame(
            {"epoch_id": [0, 1, 3], "sd1": [15, 25, 35]}
        )  # Different last epoch
    ...
    def test_load_both_tasks_valid(self, loader, valid_dyad_info):
        ...
        # Use per-task methods (new format)
        methods = {"restingstate": "nsplit1", "therapy": "nsplit120"}
        result = loader.load_both_tasks(dyad_info=valid_dyad_info, methods=methods)
...
class TestDyadPlotter:
    ...
    def sample_icd_data(self):
        ...
        return {
            "restingstate": pd.DataFrame({"epoch_id": [0], "icd_value": [50.0]}),
    ...
    def sample_dyad_info(self):
        ...
        return {"sub1": "g01p01", "ses1": "01", "sub2": "g01p02", "ses2": "01"}
    ...
    def test_plot_dyad_creates_file(
        self, plotter, sample_icd_data, sample_centroid_data, sample_dyad_info, tmp_path
    ):
    ...
    def test_calculate_trendline_with_nan(self, plotter):
        ...
        assert not np.any(np.isnan(fitted_values)), (
            "Fitted values should not contain NaN"
        )
    ...
    def test_calculate_trendline_insufficient_data(self, plotter):
        ...
        therapy_icd = pd.DataFrame(
            {"epoch_id": [0, 1, 2], "icd_value": [50.0, np.nan, np.nan]}
        )
...
class TestPlotDyadCLI:
    ...
    def test_single_dyad_mode_creates_figure(
        self, script_path, valid_dyad_pair, method, temp_output_dir
    ):
        ...
        cmd = [
            ...
            valid_dyad_pair,
            "--method",
            method,  # Primary method for therapy
            "--output-dir",
            str(temp_output_dir),
        ]
        ...
        if result.returncode != 0:
            ...
            if (
                "ICD file not found" in result.stderr
                or "Centroid file not found" in result.stderr
            ):
                pytest.skip(
                    "ICD/Centroid files not generated yet - run compute_dppa.py first"
                )
        ...
        expected_file = (
            method_dir
            / f"sub-g01p01_ses-01_vs_sub-g01p02_ses-01_method-{method}_desc-dyad_viz.png"
        )
    ...
    def test_missing_dyad_argument(self, script_path, method):
        ...
        cmd = ["uv", "run", "python", str(script_path), "--method", method]

        result = subprocess.run(cmd, capture_output=True, text=True)

        # Should fail with non-zero exit code
        assert result.returncode != 0, "Script should fail without --dyad or --batch"
        assert "Either --dyad or --batch" in result.stderr, (
    ...
    def test_help_message(self, script_path):
        ...
        assert "Generate DPPA dyadic visualizations" in result.stdout, (
            "Missing help text"
        )
